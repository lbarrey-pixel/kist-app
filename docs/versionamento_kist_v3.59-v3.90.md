...
- **[B]** `GET /crm/leads` ganha `ordenar` (whitelist: `ultimo_envio`, `proximo_followup_em`, `ultimo_contato_em`, `atualizado_em`, `criado_em`) e `crescente` — `?ordenar=proximo_followup_em&crescente=1` é a rotina do dia do bot: quem precisa ser retomado primeiro.
- **[B]** Regra de dono apertada para chave de API: `_crm_via_bot(request)` identifica se a credencial é chave (bot) ou ID token do Google (humano na tela). Pela tela, nada muda — ver é de todo mundo, só editar é do dono (ou admin). Por chave de API, agora é sempre só o próprio dono, tanto para ler (`_crm_dono_efetivo` ignora o parâmetro `dono=` e força o dono da chave) quanto para editar (`_crm_exige_dono_para_editar` não abre mais exceção pra admin quando é bot). Vale em `/crm/leads`, `/crm/leads/stream`, `/crm/leads/{dominio}`, `/crm/leads/{dominio}/contato`, `/crm/leads/{dominio}/estagio`, `/crm/painel` e as rotas novas de `/acoes`.
- DB: tabela `leads_prospeccao_acoes` (RLS ativo, só service-role, mesmo padrão das outras tabelas do CRM).
...
## v3.105 · 25/09 · Painel de desempenho do KistBot (usadas x corrigidas)
- **Pedido do Leonardo, 25/09**: quantificar o KistBot Dwight — quantas buscas dele saíram como vieram e quantas foram corrigidas, por proposta/item, por dia ou no compilado, de **todos os operadores**, atualizando sozinho a cada exportação pro Tiny, na Cabine e acessível pelo próprio bot via API.
- **Fonte**: `pesquisa_vereditos`, que já nascia na exportação (e na compra) — por isso o painel anda sozinho, sem job. Categorias: **usada** (`acertou`), **corrigida** (`escolheu_outra` / `nao_achou` = trocou de loja; `custo_divergente` = mesma loja, custo >15% longe do bot), **bot não achou** (`sem_oferta`, nova).
- **[B] Veredito por motor**: `_registrar_vereditos` pegava só o resultado MAIS RECENTE do item, de qualquer motor — item pesquisado pelo KistBot e pelo Dwight ficava com um veredito só, do que respondeu por último, e o outro sumia da conta (25/09: 17 itens nessa situação; a Tomada Zeffia da 1051042, por exemplo, não tinha veredito do KistBot). Agora cada motor tem o seu, e reexportar substitui o veredito do mesmo item/motor/momento mesmo se a pesquisa foi refeita.
- **[B] `sem_oferta`**: bot devolveu "não encontrado" (ou nenhuma oferta) e o item saiu com origem → conta contra o bot. Antes não virava veredito e sumia.
- **[B] Retrato na exportação**: `detalhe` do veredito guarda `loja_bot`, `preco_bot`, `usada`, `custo_final` do momento — o painel não depende de o item continuar igual depois. Vereditos antigos (sem retrato) leem a recomendação do resultado da pesquisa.
- **[B] `GET /pesquisa/desempenho`**: `de`/`ate` (AAAA-MM-DD, Brasília) ou `dias=N` ou nada (compilado); `operador` (opcional; padrão todos); `motor` (`kistbot` padrão, `dwight`, `todos`); `momento` (`exportacao` padrão ou `compra`); `formato=texto` pra bot. Devolve resumo, por dia, por operador e itens. Liberada na lista do escopo `pesquisa` (chave "KistBot Dwight — retorno de pesquisa"), só leitura.
- **[B]** `/pesquisa/boletim` conhece `sem_oferta` (sem isso a categoria nova derrubava o boletim com KeyError).
